...
--
-- Estas tablas se van a cargar desde hojas de cálculo Excell
-- también se exportan a Excell
...
drop TABLE tmp_ResultPresupuesto;
drop TABLE RepartoPresupuesto;
drop TABLE Presupuestos_row cascade;
...
INSERT INTO Presupuestos (Descripcion) VALUES ('Presupuesto año 2013 comunidad Los Cortijillos');
/
...
(
    id                  serial NOT NULL,
    id_ CuadernosSEPA   integer references CuadernosSEPA(id),
    registro            text
    primary key (id)
);
/
...
xIdentificaRemesa:='PRE'|| Left(TO_CHAR(NOW(),'YYYYMMDDHHMMSSSSSS'),19);

 WITH Remesas_ins AS (
    INSERT INTO Remesas (descripcion,IdentificaRemesa,fecha_cobro, cuenta_banco) 
        VALUES (xConcepto, xIdentificaRemesa,to_date(xfecha,'YYYY-MM-DD'), xCuentaBanco)
    RETURNING ID
    )
    select id into xID from Remesas_ins;

-- Candidato a un Trigger

 xValor:=''||xID;
 xValor:=lpad(xValor,13,'0');
 xIdentificaRemesa := xIdentificaRemesa || xValor;

Update Remesas SET IdentificaRemesa = xIdentificaRemesa where id = xID;
